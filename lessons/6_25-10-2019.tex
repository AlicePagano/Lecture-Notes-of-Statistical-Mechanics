...
Even if we had not seen any transition, the Ising model is interesting because we can use this model to solve other problems that seems different but are not. In fact, the importance of the Ising model relies also on the fact that it can be mapped into other discrete systems. Despite its simplicity, the Ising model is widely applicable because it describes any interacting two-state system. One of these applications is the \emph{lattice gas model}, where a gas is put in a lattice.
 ...
 Let us consider a \emph{d}-dimensional lattice with coordination number \emph{z} and lattice spacing \emph{a}, divided into cells, as in Figure \ref{fig:6_1} . Let us suppose that each cell is either empty or occupied by a single particle (this is more true if \( a \sim \SI{}{\angstrom}  \)).
...
\begin{equation}
  \mathcal{H}_ \Omega  = \sum_{i=1}^{N_c} U_1 (i) n_i + \frac{1}{2} \sum_{ij}^{N_c} U_2 (i,j) n_i n_j + O (n_i n_j n_k)
\end{equation}
...
\end{equation*}
and we will put \( U_1=0 \) for convenience.
...
\end{equation}
\begin{remark}
Note that the \( \sum_{ij} \) becomes \(z \sum_{i}\), where \emph{z} is the coordination number of neighbours.
\end{remark}
Remember that, for simplicity, we put \( U_1 = 0 \). We can rewrite:
\begin{equation}
...
\begin{equation}
  Z_{LG} = \Tr_{\{n\}}(e^{-\beta (\mathcal{H}_ \Omega -\mu N_\Omega)} ) = e^{-\beta E_0} Z_{\text{Ising}} (H,J,N_c)
\end{equation}
...
\end{equation}
\begin{remark}
Even if \( \omega _ \Omega (\dots) \) contains an infinite sum, it is not singular if \( \Omega  \) is finite!
\end{remark}
...
with the constraint
\begin{equation*}
  \rho = \lim_{V,N \rightarrow \infty } \frac{\expval{N} }{V (\Omega ) } = const
\end{equation*}
Remember also that
  \begin{equation}
...
Now:
\begin{equation}
  Z_N = \sum_{N=0}^{\infty } \frac{1}{N!} \qty[\prod_{i=1}^{N}  \qty{ \int_{-\infty }^{+ \infty } \dd[d]{\va{p}} \frac{1}{h^{dN}} e^{-\beta \va{p_i}^2/2m}  } Q_N (T) e^{ \beta \mu N}]
\end{equation}
where
\begin{equation}
...
\begin{equation*}
  \int_{-\infty }^{+ \infty } \dd[d]{\va{p}} \frac{1}{h^{d}} e^{-\beta \va{p_i}^2/2m} = \frac{1}{\Lambda (T)^d}
\end{equation*}
where
\begin{equation}
  \Lambda ( T) = \frac{h}{2 \sqrt{ \pi m k_B T} }
\end{equation}
Hence,
\begin{equation}
  Z_ \Omega = \sum_{N=0}^{\infty } \frac{1}{N!} \qty(\frac{e^{\beta \mu } }{\Lambda^d (T)})^N Q_N
  \label{eq:6_4}
\end{equation}
...
Let us divide \( \Omega  \) in discrete cells of size \emph{a}. If \emph{a} is approximate a repulsive range between particles, we have that the probability that there is more than one particle sits in a cell is \( \ll 1 \). The potentials of the continuous model depend on  \( \{ \va{r_i} \}   \).

Consider the occupation numbers \( n_ \alpha = n_ \alpha (\va{r_i}) \). We have:
\begin{equation}
  \sum_{\alpha }^{} n_ \alpha = N = \int_{\Omega}^{} \dd[d]{\va{r}} \sum_{i=1}^{N} \delta (\va{r_i}-\va{r})= \int_{\Omega}^{} \dd[d]{\va{r}} \rho (\va{r})
\end{equation}
where
\begin{equation}
...
Moreover,
\begin{equation}
  \sum_{i}^{} U_1 (\va{r_i}) = \sum_{i}^{} \int_{\Omega }^{} \dd[d]{\va{r}} U_1 (\va{r})   \delta (\va{r_i}-\va{r}) = \int_{\Omega }^{} \dd[d]{\va{r}}      U_1 (\va{r}) \rho (\va{r})
\end{equation}
...
\begin{equation*}
 Q_N \propto \int_{}^{} \prod_{i=1}^{N}  \dd[d]{\va{r_i}} \simeq N! (a^d)^{N} \sum_{\{ n_ \alpha = 0, 1 \}}^{'} \dots
\end{equation*}
\begin{remark}
The symbol \( \sum_{}^{'}   \) means that the sum has the constraint that the total number of particles is fixed to \emph{N}, that is \( \sum_{\alpha }^{} n_ \alpha = N \).
\end{remark}
Therefore,
\begin{equation}
  Q_N \propto N! (a^d)^{N} \sum_{\{ n_ \alpha  \} }^{'} e^{-\beta U ( \{ n_ \alpha  \} )}
\end{equation}
and we can rewrite the equation \eqref{eq:6_4} as 
\begin{equation}
  Z_ \Omega  = \sum_{N=0}^{\infty } \frac{1}{N!} \qty(\frac{e^{\beta \mu } }{\Lambda ^d (T)}) ^N Q_N = \sum_{N=0}^{\infty } \qty[e^{\beta \mu} \qty( \frac{a}{\Lambda (T)}  )^d ]  ^N \sum_{\{ n_ \alpha  \} }^{'} e^{-\beta U ( \{ n_ \alpha  \} )}
\end{equation}
where \( \sum_{}^{'} = \sum_{\{ n_ \alpha  \} }^{}     \)  with the constraint \( \sum_{\alpha }^{} n_ \alpha = N   \).
\begin{remark}
In general it is difficult to perform sum with constraints. Fortunately, we are considering the gran-canonical ensemble.
Indeed, we can write
\begin{equation*}
...
\end{remark}
Eventually, we have\footnote{ \( b^a = e^{a \ln b} \, \Rightarrow e^{\beta \mu} ( \frac{a}{\Lambda})^d = e^{\beta \mu} \exp (d \ln (\frac{a}{\Lambda}) ) \) }
\begin{equation}
...
\begin{figure}[h!]
\centering
\includegraphics[width=0.7\textwidth]{../lessons/6_image/3.pdf}
