...
\begin{equation}
  w (\sigma _i, \sigma _j) = - \hat{g} - \frac{\hat{h} }{z} (\sigma _i + \sigma _j) - \hat{J} \sigma _i \sigma _j
\end{equation}
...
The idea of the renormalization group is to perform an integration over some degrees of freedom and obtain a new partition function with \( N/b^d \) spins but of the same form of the one describing the original system with \( N \) spins.

There are many ways to perform this partial integration. Here we consider the \emph{decimation} procedure with \( b=2 \).

\subsection{Ising \( \pmb{d=1} \), RSRG with \( \pmb{b=2} \).}
...
\begin{equation*}
  Z(g,k,h) = \sum_{\{ \sigma _i \}_i^N  }^{}  e^{\sum_{i}^{}  w (\sigma _i, \sigma _{i+1})}
...
\begin{equation}
  Z = \sum_{\{ \sigma _i' \}_1^{N/2}} \prod_{i=1}^{N/2}  \qty(  \sum_{ \{ S_i=\pm 1 \} }^{}  e^{ \qty[ w(\sigma _i',S_i) + w(S_i, \sigma _{i+1}') ]  }  )
\end{equation}
