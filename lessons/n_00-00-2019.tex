...
\end{equation*}
The \( \mathcal{L} (\Phi ) \) describes a scalar complex (i.e. charged) \emph{muonic field} with mass \( m \). Let us note that in this case we have \( r_0>0 \) and the mass \( m \) is given by \( m \equiv \sqrt{r_0}  \). Moreover, the term \( (\Phi \Phi ^*)^2 \) means self-interaction with strenght \( \lambda \equiv u_0 \).

In all cases, the original symmetry is \( U(1) \); it means that both the Hamiltonian \( \mathcal{H} \) and \( \mathcal{L} \) are invariant with respect to the transformations
\begin{equation*}
